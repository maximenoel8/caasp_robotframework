...
velero deployment for gcp
    ...
    helm    install --name velero --namespace velero --set configuration.provider=gcp --set configuration.backupStorageLocation.name=default --set configuration.backupStorageLocation.bucket=${BUCKET_MASTER} --set snapshotsEnabled=true --set deployRestic=true --set configuration.volumeSnapshotLocation.name=default --set-file credentials.secretContents.cloud=${DATADIR}/${gcp_credential_file} --set initContainers[0].name=velero-plugin-for-gcp --set initContainers[0].image=${gcp_plugin_image} --set initContainers[0].volumeMounts[0].mountPath=/target --set initContainers[0].volumeMounts[0].name=plugins suse-charts/velero    ${cluster_number}
