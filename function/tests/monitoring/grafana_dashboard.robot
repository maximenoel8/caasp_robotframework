...
access certificate dashboard
    ...
    FOR    ${grafana_panel_element}    IN    @{grafana_panels_webelements}
        ...
        ${exp_7d_grafana_panels_webelement}    Set Variable If    "${panel_title}"=="Expiration within 7 days"    ${grafana_panel_element}    ${exp_7d_grafana_panels_webelement}
        ${exp_7dT30d_grafana_panels_webelement}    Set Variable If    "${panel_title}"=="Expiration within 7 and 30 days"    ${grafana_panel_element}    ${exp_7dT30d_grafana_panels_webelement}
    ...
    _test expiration within 7days panel    ${exp_7d_grafana_panels_webelement}    ${node_number}    ${master_number}
    _test expiration within 7 and 30 days panel    ${exp_7dT30d_grafana_panels_webelement}    ${node_number}    ${master_number}
    _test expiration after 30 days panel    ${exp_30d_grafana_panels_webelement}    ${node_number}    ${master_number}

_test expiration within 7days panel
...
_test expiration within 7 and 30 days panel
    [Arguments]    ${panel_webelement}    ${node_number}    ${master_number}
    ${certificate_number}    get child webelements    ${panel_webelement}    .singlestat-panel-value-container span
    ${certificate_number_text}    Get Text    ${certificate_number[0]}
    Should Be Equal As Integers    ${certificate_number_text}    0
...
_test kubernetes and etcd pki certs expiry panel
    ...
    ${expected_etcd_certificates_number}    Evaluate    3 * ${node_number} + 9 * \ ${master_number}
...
_test kubeconfig certs expiry panel
    ...
    @{raws}    get child webelements    ${panel_webelement}    tbody tr
    ${raw_number}    Get Length    ${raws}
    ${expected_etcd_certificates_number}    Evaluate    ${node_number} + 6 * \ ${master_number}
    Should Be Equal As Integers    ${raw_number}    ${expected_etcd_certificates_number}
    FOR    ${element}    IN    @{raws}
...
_check value from json kubernetes and etcd
    ...
    Should Be Equal    ${sub_dico["filename"]}    ${filename}
